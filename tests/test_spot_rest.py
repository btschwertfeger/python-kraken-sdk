...
class UserTests(unittest.TestCase):
    ...
    def test_get_closed_orders(self):
        ...
        assert is_not_error(self.__auth_user.get_closed_orders(trades=True, start='1668431675.4778206'))
        assert is_not_error(self.__auth_user.get_closed_orders(
    ...
    def test_get_orders_info(self):
        for params, method in zip([
            ...
            self.__auth_user.get_orders_info, self.__auth_user.get_orders_info,
            self.__auth_user.get_trades_info, self.__auth_user.get_trades_info
        ]):
    ...
    def test_get_trades_history(self):
            assert is_not_error(self.__auth_user.get_trades_history(type_='all', trades=True))

    def test_get_open_positions(self):
        assert isinstance(self.__auth_user.get_open_positions(), list)
        assert isinstance(self.__auth_user.get_open_positions(txid='OQQYNL-FXCFA-FBFVD7'), list)
        assert isinstance(self.__auth_user.get_open_positions(txid='OQQYNL-FXCFA-FBFVD7', docalcs=True), list)
    ...
    def test_get_ledgers_info(self):
        ...
        assert is_not_error(self.__auth_user.get_ledgers_info(asset='EUR', start='1668431675.4778206', end='1668455555.4778206', ofs=2))

    def test_get_ledgers(self):
        assert is_not_error(self.__auth_user.get_ledgers(id_='LNYQGU-SUR5U-UXTOWM'))
        assert is_not_error(self.__auth_user.get_ledgers(id_=['LNYQGU-SUR5U-UXTOWM', 'LTCMN2-5DZHX-6CPRC4'], trades=True))

    def test_get_trade_volume(self):
        assert is_not_error(self.__auth_user.get_trade_volume())
        assert is_not_error(self.__auth_user.get_trade_volume(pair='DOT/EUR', fee_info=False))
    ...
    def test_request_save_export_report(self):
        for report in ['trades', 'ledgers']:
            ...
            for response in status:
                ...
                try:
                    assert isinstance(self.__auth_user.delete_export_report(id_=response['id'], type_='delete'), dict)
...
class MarketTests(unittest.TestCase):
    ...
    def test_get_tradable_asset_pair(self): 
        ...
        assert is_not_error(self.__market.get_tradable_asset_pair(pair=['DOTEUR', 'BTCUSD']))
        for i in ['info', 'leverage', 'fees', 'margin']:
            assert is_not_error(self.__market.get_tradable_asset_pair(pair='DOTEUR', info=i))
            break
    ...
    def test_get_ohlc(self):
        ...
        assert is_not_error(self.__market.get_ohlc(pair='XBTUSD', interval=240)) # interval in [1 5 15 30 60 240 1440 10080 21600]

    def test_get_order_book(self):
        assert is_not_error(self.__market.get_order_book(pair='XBTUSD'))
        assert is_not_error(self.__market.get_order_book(pair='XBTUSD', count=2)) # count in [1...500]

    def test_get_recent_trades(self):
        assert is_not_error(self.__market.get_recent_trades(pair='XBTUSD'))
        assert is_not_error(self.__market.get_recent_trades(pair='XBTUSD', since='1616663618'))

    def test_get_recend_spreads(self):
        assert is_not_error(self.__market.get_recend_spreads(pair='XBTUSD'))
        assert is_not_error(self.__market.get_recend_spreads(pair='XBTUSD', since='1616663618'))
...
class TradeTests(unittest.TestCase):
    ...
    def test_create_order(self):
        assert isinstance(self.__auth_trade.create_order(
            ordertype='limit',
            side='buy',
            volume=1,
            pair='BTC/EUR',
    ...
    def test_create_order_batch(self):
        assert isinstance(self.__auth_trade.create_order_batch(
            orders=[{
            'close': {
    ...
    def test_edit_order(self):
        assert isinstance(self.__auth_trade.edit_order(
            txid='sometxid',
            pair='BTC/EUR',
            volume=4.2,
            price=17000,
    ...
    def test_cancel_order(self):
        assert isinstance(self.__auth_trade.cancel_order(txid='O2JLFP-VYFIW-35ZAAE'), dict)
...
class StakingTests(unittest.TestCase):
    ...
    def test_stake_asset(self):
        try:
            assert is_not_error(self.__auth_staking.stake_asset(asset='DOT', amount='4500000', method='polkadot-staked'))
...
class FundingTests(unittest.TestCase):
    ...
    def test_withdraw_funds(self):
        try:
            assert is_not_error(self.__auth_funding.withdraw_funds(asset='XLM', key='enter-withdraw-key', amount=100000))
    ...
    def test_get_recend_withdraw_status(self):
        assert isinstance(self.__auth_funding.get_recend_withdraw_status(asset='XLM'), list)
        try:
            assert is_not_error(
